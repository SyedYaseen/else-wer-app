...
export interface DownloadState {
  ...
  addToQueue: (payload: { bookId: number, fileId: number, fileName: string }) => void
  setProgress: (bookId: number, fileId: number, progress: number) => void,
  setManagerRef: (m: DownloadManagerApi) => void
  startDownload: (payload: { bookId: number; fileId: number; fileName: string }) => Promise<string | null>
...
export const useDownloadStore = create<DownloadState>()(subscribeWithSelector((set, get) => ({
  ...
  addToQueue: ({ bookId, fileId, fileName }) => set((s) => {
