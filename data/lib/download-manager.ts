...
export class DownloadManager {
    ...
    private async downloadFileInChunks(bookId: number, fileId: number, fileName?: string) {
        ...
        await FileSystem.makeDirectoryAsync(`${ROOT}${bookId}`, { intermediates: true }).catch(() => { })
        const finalFileName = fileName || `${fileId}.mp3`
        const destPath = `${ROOT}${bookId}/${finalFileName}`
        ...
        const worker = async () => {
            while (pointer < chunks.length) {
                ...
                const progress = Math.min(1, (pointer / chunks.length))
                // update fast in-memory progress
                store.setProgress(bookId, fileId, progress)
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            await FileSystem.writeAsStringAsync(destPath, b64, {
                ...
                // @ts-expect-error: append supported at runtime
                append: true,
            })
        }
